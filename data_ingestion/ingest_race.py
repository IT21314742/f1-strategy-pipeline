...
class F1DataPipeline:
    ...
    def ingest_race(self, year, grand_prix):
        ...
        self.cursor.execute("""
            ...
            RETURNING race_id
        """, (season_id, int(race.event['RoundNumber']), 
        ...
        for driver_code in race.drivers:
            ...
            """, (driver_code, driver_info['FullName'], team_id, 
                  int(driver_info.get('DriverNumber', 0)) if driver_info.get('DriverNumber') else 0))
        ...
        for _, lap in laps.iterrows():
            ...
            lap_data.append((
                ...
                sector2,
                sector3,
                int(lap['Position']) if lap['Position'] and not pd.isna(lap['Position']) else None,
                compound,
                int(lap.get('TyreLife', None)) if lap.get('TyreLife') and not pd.isna(lap.get('TyreLife')) else None
            ))
